...
#Когда добавлю условие, которое проверяет ALT на нажатие, почему-то не срабатывает условие.
#Курсор также перемещается блогодоря стрелкам при нажатом alt.
#Когда я убераю условие, то alt работает корректно, но кусрор не перемещается.
#Нужно потом попробывать с дополнительными условиями.
